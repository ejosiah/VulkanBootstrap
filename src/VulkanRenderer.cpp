...
void VulkanRenderer::renderFrame() {
    vkWaitForFences(*_device, 1, &_inFlightFrame, false, UINT64_MAX);
    vkResetFences(*_device, 1, &_inFlightFrame);

    uint32_t imageIndex = ~0u;
    auto status = vkAcquireNextImageKHR(*_device, *_swapchain, UINT64_MAX, _acquireImageSemaphore, VK_NULL_HANDLE, &imageIndex);
    ...
    recordScene(imageIndex);

    _renderSubmitInfo.pCommandBuffers = &_renderCommandBuffer[imageIndex];
    vkQueueSubmit(_device->getGraphicsQueue(), 1, &_renderSubmitInfo, _inFlightFrame);
...
void VulkanRenderer::createCommandPool() {
    ...
    _renderCommandBuffer = _commandPool->allocate(_swapchain->imageCount());
}

void VulkanRenderer::createSynchronizationPrimitives() {
    VkSemaphoreCreateInfo semaphoreCreateInfo{ .sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO };

    auto result = vkCreateSemaphore(_device->handle(), &semaphoreCreateInfo, nullptr, &_acquireImageSemaphore);
    if(result != VK_SUCCESS) {
        throw std::runtime_error{ "unable to create acquireImageSemaphore" };
    }
    result = vkCreateSemaphore(_device->handle(), &semaphoreCreateInfo, nullptr, &_renderingFinishedSemaphore);
    if(result != VK_SUCCESS) {
    ...
    result = vkCreateFence(_device->handle(), &fenceCreateInfo, nullptr, &_inFlightFrame);
    if(result != VK_SUCCESS) {
    ...
    _renderSubmitInfo.pWaitSemaphores = &_acquireImageSemaphore;
    _renderSubmitInfo.commandBufferCount = 1;
    _renderSubmitInfo.signalSemaphoreCount = 1;
    _renderSubmitInfo.pSignalSemaphores = &_renderingFinishedSemaphore;

    _presentInfo = { VK_STRUCTURE_TYPE_PRESENT_INFO_KHR };
    _presentInfo.waitSemaphoreCount = 1;
    _presentInfo.pWaitSemaphores = &_renderingFinishedSemaphore;
...
void VulkanRenderer::recordScene(uint32_t imageIndex) {
    VkCommandBufferBeginInfo beginInfo{ VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO };
    vkBeginCommandBuffer(_renderCommandBuffer[imageIndex], &beginInfo);
    vkCmdBeginRendering(_renderCommandBuffer[imageIndex], &_renderingInfo);

    vkCmdEndRendering(_renderCommandBuffer[imageIndex]);
    vkEndCommandBuffer(_renderCommandBuffer[imageIndex]);
}
...
void VulkanRenderer::stop() {
    vkDestroySemaphore(_device->handle(), _acquireImageSemaphore, VK_NULL_HANDLE);
    vkDestroySemaphore(_device->handle(), _renderingFinishedSemaphore, VK_NULL_HANDLE);
    vkDestroyFence(_device->handle(), _inFlightFrame, VK_NULL_HANDLE);
}
...
void VulkanRenderer::invalidateSwapchain() {
    ...
    _device->wait();
    vkDestroySemaphore(*_device, _acquireImageSemaphore, VK_NULL_HANDLE);
    vkDestroySemaphore(*_device, _renderingFinishedSemaphore, VK_NULL_HANDLE);
    vkDestroyFence(*_device, _inFlightFrame, VK_NULL_HANDLE);
